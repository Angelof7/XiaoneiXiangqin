...
<%@ page import="java.io.*,com.xiangqin.ORM.PersonalInfo;"%>
<%
...
<title>${otherInfo.nickName }的个人主页</title>
...
<body class="profile profile-male" reload="true" onload="change()">
			...
			<div id="profileBasicMain" class="profile-basic-main">
				<div id="userStatusBar" class="profile-basic-verified">
					<span id="profileVerifyName"
						class="profile-basic-name js-nickname ">${otherInfo.nickName}</span>
					<span class="profile-basic-id">ID：${otherInfo.userId }</span>
				</div>
				<div class="profile-basic-info">
					<div class="profile-basic-info-left">
						<span><c:choose>
								<c:when test="${otherInfo.gender == 0}">
									<c:out value="男" />
								</c:when>
								<c:otherwise>
									<c:out value="女"></c:out>
								</c:otherwise>
							</c:choose> </span> <span><em class="js-age">${otherInfo.age}</em>岁</span> <span><em
							class="js-height">${otherInfo.height}</em>cm</span> <span
							class="js-position">${otherInfo.liveLocation}</span>
					</div>
				...
				<div class="profile-basic-relation clearfix js-relation-btn"
					data-isBlacked="false">
					<span class="btn-cancel n-btn-cancelBlock"> <span
						class="btn-cancel-txt">已拉黑</span> <a
						class="btn-cancel-link cancelBlock-trigger" href="javascript:;">解除</a>
					</span> <a class="btn-mess mess-trigger n-btn-mess" href="javascript:;">
						<span class="btn-mess-txt"><em class="icon-mess-s"></em>私信</span>
						...
						class="btn-fav-box-inner js-like"> <a
							class="btn-fav like-trigger" href="javascript:;"> <span
								...
								class="btn-fav-disabled-unfav cancelLike-trigger">取消</span>
						</a>
					</span>
	...
	<div class="contentBox" style="margin-top: 0px; background: white">
		个人形象照展示<br> <img alt=""
			src="upload/avatar/${otherInfo.userId }/${otherInfo.userId }_162.jpg">
	</div>
	<div class="contentBox">
		<div class="tab-container" id="tab-container">
			<ul class="tabBar">
				<li class="tab"><a href="#person">TA的动态</a></li>
				<li class="tab"><a href="#form1">TA的资料</a></li>
				<li class="tab"><a href="#form2" onclick="loadContent();">TA的相册</a></li>
			</ul>
			<div class="panel-container">
				...
				<div id="form1">
					<div class="PersonalInfo">
						<li><span>昵称：</span>${otherInfo.nickName}</li>
						<li><span>婚姻状态：</span> <c:choose>
								<c:when test="${otherInfo.marriageStatus==0}">
									<c:out value="请选择"></c:out>
								</c:when>
								<c:when test="${otherInfo.marriageStatus==1}">
						...
						<li><span>我在寻找：</span> <c:if
								test="${otherInfo.objectType == 0}">
								<c:out value="结婚对象"></c:out>
							</c:if> <c:if test="${otherInfo.objectType == 1}">
								<c:out value="恋人"></c:out>
							</c:if> <c:if test="${otherInfo.objectType == 2}">
								<c:out value="普通朋友"></c:out>
							</c:if> <c:if test="${otherInfo.objectType == 3}">
						...
						<li><span>年龄：</span>${otherInfo.age}岁</li>
						<li><span>身高：</span>${otherInfo.height}cm</li>
						<li><span>体重：</span>${otherInfo.weight}kg</li>
						<li><span>居住在：</span>${otherInfo.liveLocation}</li>
						<li><span>学历：</span> <c:if test="${otherInfo.education == 0}">
						...
						<li><span>毕业院校：</span>${otherInfo.graduateFrom}</li>
						<li><span>行业：</span> <c:choose>
								<c:when test="${otherInfo.industry == 0}">
									<c:out value="请选择"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 1}">
									<c:out value="计算机/互联网/通信"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 2}">
									<c:out value="公务员/事业单位"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 3}">
									<c:out value="教师"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 4}">
									<c:out value="医生"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 5}">
									<c:out value="护士"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 6}">
									<c:out value="乘务员"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 7}">
									<c:out value="生产/工艺/制造"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 8}">
									<c:out value="商业/服务业/个体经营"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 9}">
									<c:out value="金融/银行/投资/保险"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 10}">
									<c:out value="文化/广告/传媒"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 11}">
									<c:out value="娱乐/艺术/表演"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 12}">
									<c:out value="律师/法务"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 13}">
									<c:out value="教育/培训/管理咨询"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 14}">
									<c:out value="建筑/房地产/物业"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 15}">
									<c:out value="消费零售/贸易/物流"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 16}">
									<c:out value="酒店/旅游"></c:out>
								</c:when>
								<c:when test="${otherInfo.industry == 17}">
						...
						<li><span>工作单位：</span>${otherInfo.company}</li>
						<li><span>目前职位：</span> <c:choose>
								<c:when test="${otherInfo.currentJob==0}">
									<c:out value="请选择"></c:out>
								</c:when>
								<c:when test="${otherInfo.currentJob==1}">
									<c:out value="普通职员"></c:out>
								</c:when>
								<c:when test="${otherInfo.currentJob==2}">
									<c:out value="中层管理者"></c:out>
								</c:when>
								<c:when test="${otherInfo.currentJob==3}">
						...
						<li><span>月收入：</span> <c:if
								test="${otherInfo.monthlyIncome == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 1}">
								<c:out value="2000-4000"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 2}">
								<c:out value="2000-4000"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 3}">
								<c:out value="4000-6000"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 4}">
								<c:out value="6000-10000"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 5}">
								<c:out value="10000-15000"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 6}">
								<c:out value="15000-20000"></c:out>
							</c:if> <c:if test="${otherInfo.monthlyIncome == 7}">
								<c:out value="20000以上"></c:out>
							</c:if></li>
						<li><span>户口：</span>${otherInfo.account}</li>
						<li><span>住房情况：</span> <c:if
								test="${otherInfo.housingCondition == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.housingCondition == 1}">
								<c:out value="已购房"></c:out>
							</c:if> <c:if test="${otherInfo.housingCondition == 2}">
								<c:out value="租房"></c:out>
							</c:if> <c:if test="${otherInfo.housingCondition == 3}">
								<c:out value="单位宿舍"></c:out>
							</c:if> <c:if test="${otherInfo.housingCondition == 4}">
								<c:out value="和家人同住"></c:out>
							</c:if></li>
						<li><span>购车情况：</span> <c:if
								test="${otherInfo.carCondition == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.carCondition == 1}">
								<c:out value="已购车"></c:out>
							</c:if> <c:if test="${otherInfo.carCondition == 2}">
								<c:out value="未购车"></c:out>
							</c:if></li>
						<li><span>民族：</span>${otherInfo.ethnic}</li>
						<li><span>籍贯：</span>${otherInfo.birthLocation}</li>
						<li><span>家中排行：</span> <c:if
								test="${otherInfo.homeRanking == 0}">
								<c:out value="独生子女"></c:out>
							</c:if> <c:if test="${otherInfo.homeRanking == 1}">
								<c:out value="老大"></c:out>
							</c:if> <c:if test="${otherInfo.homeRanking == 2}">
								<c:out value="老二"></c:out>
							</c:if> <c:if test="${otherInfo.homeRanking == 3}">
								<c:out value="老三及以后"></c:out>
							</c:if></li>
						<li><span>有无子女： </span> <c:if
								test="${otherInfo.haveChildren == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.haveChildren == 1}">
								<c:out value="小孩归自己"></c:out>
							</c:if> <c:if test="${otherInfo.haveChildren == 2}">
								<c:out value="小孩归对方"></c:out>
							</c:if></li>
						<li><span>星座： </span> <c:if
								test="${otherInfo.constellation == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 1}">
								<c:out value="水瓶座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 2}">
								<c:out value="双鱼座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 3}">
								<c:out value="白羊座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 4}">
								<c:out value="金牛座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 5}">
								<c:out value="双子座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 6}">
								<c:out value="巨蟹座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 7}">
								<c:out value="狮子座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 8}">
								<c:out value="处女座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 9}">
								<c:out value="天秤座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 10}">
								<c:out value="天蝎座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 11}">
								<c:out value="射手座"></c:out>
							</c:if> <c:if test="${otherInfo.constellation == 12}">
								<c:out value="摩羯座"></c:out>
							</c:if></li>
						<li><span>血型：</span> <c:if test="${otherInfo.bloodType == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.bloodType == 1}">
								<c:out value="A型"></c:out>
							</c:if> <c:if test="${otherInfo.bloodType == 2}">
								<c:out value="B型"></c:out>
							</c:if> <c:if test="${otherInfo.bloodType == 3}">
								<c:out value="AB型"></c:out>
							</c:if> <c:if test="${otherInfo.bloodType == 4}">
								<c:out value="O型"></c:out>
							</c:if></li>
						<li><span>属相： </span> <c:if test="${otherInfo.zodiac == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 1}">
								<c:out value="鼠"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 2}">
								<c:out value="牛"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 3}">
								<c:out value="虎"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 4}">
								<c:out value="兔"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 5}">
								<c:out value="龙"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 6}">
								<c:out value="蛇"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 7}">
								<c:out value="马"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 8}">
								<c:out value="羊"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 9}">
								<c:out value="猴"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 10}">
								<c:out value="鸡"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 11}">
								<c:out value="狗"></c:out>
							</c:if> <c:if test="${otherInfo.zodiac == 12}">
								<c:out value="猪"></c:out>
							</c:if></li>
						<li><span>宗教信仰：</span> <c:if
								test="${otherInfo.religion == 0}">
								<c:out value="请选择"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 1}">
								<c:out value="无宗教信仰"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 2}">
								<c:out value="佛教"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 3}">
								<c:out value="道教"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 4}">
								<c:out value="儒教"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 5}">
								<c:out value="基督教"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 6}">
								<c:out value="犹太教"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 7}">
								<c:out value="伊斯兰教"></c:out>
							</c:if> <c:if test="${otherInfo.religion == 8}">
								<c:out value="其他信仰"></c:out>
							</c:if></li>
						<li><span>微博地址：</span>${otherInfo.weiboURL}</li>
						<li><span>豆瓣地址：</span>${otherInfo.doubanURL}</li>
