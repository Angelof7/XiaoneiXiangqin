...
public class VerificationId {
	...
	public boolean verificationId() throws IOException {
		...
		if (studentN.equals("MG") || studentN.equals("MF")
				|| studentN.equals("MP") || studentN.equals("DG")) {
			set2StudentLevel(studentN);
			studentYear = (currentYear / 1000 * 1000)
					+ Integer.parseInt(studentYear) + "";
			int tempYear = Integer.parseInt(studentYear);
			System.out.println(studentYear + " " + tempYear);
			if (currentYear - tempYear <= timeLimited) {
	...
	public void set2StudentAcademy(String studentAcad) throws IOException {
		InputStream file = VerificationId.class
				.getResourceAsStream("academy.txt");
		String line;
		String[] str;
